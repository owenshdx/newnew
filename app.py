...
st.markdown("""
    ...
    .breakdown-label { font-size: 11px; color: #94a3b8; font-weight: 500; }
    .breakdown-badge {
        font-size: 9px;
        font-weight: 800;
        padding: 2px 8px;
    ...
    .optix-table tr:hover { background: rgba(30, 41, 59, 0.2); }
    .unusual-row { background: rgba(37, 99, 235, 0.05); }
    .unusual-vol { color: #3b82f6; font-weight: 800; }

    /* Indicator Colors */
    .val-bullish { background: rgba(34, 197, 94, 0.1); color: #4ade80; border: 1px solid rgba(34, 197, 94, 0.2); }
    .val-bearish { background: rgba(239, 68, 68, 0.1); color: #f87171; border: 1px solid rgba(239, 68, 68, 0.2); }
    .val-neutral { background: rgba(30, 41, 59, 0.5); color: #94a3b8; border: 1px solid rgba(71, 85, 105, 0.3); }
    .val-warning { background: rgba(245, 158, 11, 0.1); color: #fbbf24; border: 1px solid rgba(245, 158, 11, 0.2); }
    ...
    .cal-top { font-size: 4px; background: #ef4444; color: white; width: 100%; text-align: center; }
    .cal-day { font-size: 10px; font-weight: 800; }

    /* Sidebar Fixes */
    [data-testid="stSidebar"] { background-color: #020617; border-right: 1px solid #1e293b; }
    .stSelectbox div[data-baseweb="select"] { background-color: #0f172a; border-color: #1e293b; }
    </style>
...
def get_scores(df, options, earnings_days):
    ...
    if rsi < 35: c_s += 15; b['rsi'] = f"{int(rsi)} Oversold (+15c)"
    elif rsi > 65: p_s += 15; b['rsi'] = f"{int(rsi)} Overbought (+15p)"
...
# --- CUSTOM RENDERERS ---
def render_options_table(df, filter_unusual=False):
    if filter_unusual: df = df[df['isUnusual']]
    df = df.head(10)
    html = '<table class="optix-table"><thead><tr><th>Strike</th><th>Price</th><th>Vol</th><th>OI</th><th style="text-align:right">IV</th></tr></thead><tbody>'
    for _, row in df.iterrows():
        ...
        oi_val = f"{row['openInterest']/1000:.1f}k" if row['openInterest'] > 1000 else int(row['openInterest'])
        html += f'''<tr class="{cls}">
            <td style="font-weight:700; color:#cbd5e1;">{row['strike']:.1f}</td>
            <td>${row['lastPrice']:.2f}</td>
            <td class="{vol_cls}">{vol_val}</td>
            <td style="color:#64748b;">{oi_val}</td>
...
# --- APP UI ---
def main():
    st.sidebar.markdown("<div style='font-size: 9px; font-weight: 900; color: #64748b; letter-spacing: 2px; margin-bottom: 20px;'>MARKET DIRECTORY</div>", unsafe_allow_html=True)
    ...
    st.markdown(f"""
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 15px 0 30px 0; border-bottom: 1px solid #1e293b; margin-bottom: 25px;">
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <h1 style="font-size: 1.4rem; font-weight: 800; margin: 0; letter-spacing: -0.5px;">Optix <span style="color:#475569; font-size:0.75rem; letter-spacing:1px; margin-left:10px; font-weight:600;">PRO TERMINAL</span></h1>
            </div>
            <div style="display: flex; align-items: center; gap: 12px;">
                {f'<div style="font-size:9px; background:rgba(180,83,9,0.1); color:#fbbf24; padding:4px 10px; border-radius:15px; border:1px solid rgba(180,83,9,0.2);">SIMULATION ACTIVE</div>' if df_is_mock else ''}
                <div style="font-size: 10px; font-weight: 800; padding: 5px 12px; border-radius: 20px; background-color: {'rgba(34,197,94,0.1)' if isOpen else 'rgba(239,68,68,0.1)'}; color: {'#4ade80' if isOpen else '#f87171'}; border: 1px solid {'rgba(34,197,94,0.2)' if isOpen else 'rgba(239,68,68,0.2)'};">● {status_str}</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    with col_l:
        ...
        st.markdown(f"""
            <div class="optix-card">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                    ...
                    <span style="font-size: 9px; font-weight: 900; background: {'#2563eb' if strength == 'Strong' else '#1e293b'}; color: white; padding: 3px 12px; border-radius: 20px;">{strength.upper()}</span>
                </div>
                <div class="signal-container">
                    <div class="signal-bar-wrapper">
                        <span class="signal-bar-value" style="color:#4ade80;">{c_s}%</span>
                        <div class="signal-bar" style="height: {c_s}%; background: rgba(34,197,94,0.1); border-top: 2px solid #22c55e;"></div>
                        <div class="signal-label">CALL BIAS</div>
                    </div>
                    <div class="signal-bar-wrapper">
                        <span class="signal-bar-value" style="color:#f87171;">{p_s}%</span>
                        <div class="signal-bar" style="height: {p_s}%; background: rgba(239,68,68,0.1); border-top: 2px solid #ef4444;"></div>
                ...
                <div style="margin-top:20px; border-top:1px solid #1e293b; padding-top:15px;">
                    ...
                    <div class="breakdown-row">
                        ...
                        <div style="display:flex; align-items:center; gap:10px;">
                            <div class="mini-calendar"><div class="cal-top">T-MINUS</div><div class="cal-day">{ed_raw}</div></div>
        ...
        st.markdown(f"""
            <div class="optix-card" style="padding: 20px;">
                <div style="font-size: 9px; font-weight: 900; color: #475569; margin-bottom: 5px; letter-spacing: 1px;">{selected_ticker} SPOT</div>
                <div style="font-size: 1.8rem; font-weight: 900; color: #f1f5f9; letter-spacing: -1px;">${price:.2f}</div>
            </div>
        """, unsafe_allow_html=True)
    ...
    with col_r:
        ...
        fig.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=500, xaxis_rangeslider_visible=False, margin=dict(l=0, r=40, t=10, b=10), showlegend=False, font=dict(family="Inter", size=10))
        fig.update_yaxes(side='right', gridcolor='rgba(30,41,59,0.5)', row=1, col=1)
        fig.update_yaxes(side='right', gridcolor='rgba(30,41,59,0.5)', range=[0, 100], row=2, col=1)
        
        st.markdown('<div class="optix-card" style="padding: 12px; height: 530px;">', unsafe_allow_html=True)
        st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="optix-card" style="padding: 0; overflow: hidden;">', unsafe_allow_html=True)
        t_header_col, t_filter_col = st.columns([1, 1])
        with t_header_col: st.markdown('<div style="padding: 15px 20px; font-size: 10px; font-weight: 900; color: #64748b; letter-spacing: 1px;">OPTIONS ACTIVITY</div>', unsafe_allow_html=True)
        with t_filter_col: filter_unusual = st.checkbox("Unusual Vol Only", value=False)
        
        tc1, tc2 = st.tabs(["CALLS", "PUTS"])
        with tc1: st.markdown(render_options_table(opts_raw['calls'], filter_unusual), unsafe_allow_html=True)
        with tc2: st.markdown(render_options_table(opts_raw['puts'], filter_unusual), unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
